...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const { data } = await httpClient.get<TProductData>(
    `/products/public?limit=all`
  );
  const { data: catalog } = await httpClient.get<TCategoryData>(
    "/categories/public?isParent=true&limit=all"
  );

  const singleProductPages: MetadataRoute.Sitemap = data?.data?.map(
    ({ slug }) => ({
