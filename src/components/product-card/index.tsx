...
export const ProductCard: FC<IProductCardItemsType> = (props) => {
  ...
  return (
    <div className="relative w-[159px] xs:w-full xs:h-[308px] h-[270px] rounded-[12px] bg-ClrEAE9 shadow-cardShadow">
      <Image className="rounded-t-[12px] xs:w-full xs:h-[190px]" src={img} alt={title} />
      <h3 className="text-[16px] pl-[9px] py-[5px] border-b-[1px] border-white text-Clr4002 font-light">
        {t(title)}
      </h3>
      <div className="px-[9px]">
        <div className="flex justify-between items-center">
          <p className="text-Clr4002 text-[14px] xs:text-[18px]">{t(salePrice)} won</p>
          <strong className="text-ClrA5A1 text-[12px] xs:text-[16px] font-medium line-through">
            {t(price.toUpperCase())} w
          </strong>
        </div>
        ...
        <div className="flex items-center gap-x-1 mt-2">
          <CardLocationIcon />
          <p className="text-Clr4002 font-light text-[12px] xs:text-[16px]">{t(restaurantName)}</p>
        </div>
